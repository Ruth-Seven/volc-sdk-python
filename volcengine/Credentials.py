...
class Credentials:
    ...
    def set_config(self, credential_section=DEFAULT_SETTION):
        ...
        credential_ak, credential_sk = self._load_file_config(
            self.credential_file,
            credential_section,
            "access_key_id",
            "secret_access_key",
        )
        (config_region_custom,) = self._load_file_config(
            self.config_file, DEFAULT_CONFIG_SETION, "region"
        )
        (config_region_default,) = self._load_file_config(
            self.config_file, DEFAULT_SETTION, "region"
        )
    ...
    def _fifo_valid_value(self, *values):
        for value in values:
            if value:
                return value
        raise ValueError(
            "Each value is null or empty. Please check the valid parameters"
        )
